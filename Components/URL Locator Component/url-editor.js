...
Framework.Components.registerComponent("input[type=url]", {
	growTimer: false,
	updateTimer: false,
	searchResults: false,
	...
	updateResultsRaw: function(html) {
		try {
			...
			this.searchResults.select("a").each((function(anchor) {
				anchor.on("mousedown",(function(e) {
					...
					activeSearchComponent.setValue(cur.readAttribute("target"));
					this.hideResults();
					e.stop();
				}).bind(this));
			}).bind(this));
		...
		switch(e.keyCode) {
			...
			case 38: // Up
				...
				if(cur) {
					...
					cur = this.searchResults.select("a");
